...
public class RaportAnTables {
	...
	public static List<Modul> getModuleStudent(int id_student, int id_disciplina) {
		...
		try {
			session = Singleton.getInstance().getNewSession();
			list = ModulService.getAllModulesForStudentAndDisciplina(
					StudentService.getStudentByID(id_student),
					DisciplinaService.getDisciplinaByID(id_disciplina));
	...
	public static void MakeYearReport(int year) {
		...
		try {
			PdfWriter writer = PdfWriter
					.getInstance(document, new FileOutputStream(new File(
							"Tabel_an_" + year + ".pdf")));
			...
			Paragraph header = new Paragraph("Situatia scolara a studentilor");
			...
			Paragraph an = new Paragraph("Anul de studii: " + year + "\n");
			an.setSpacingAfter(15);
			an.setSpacingBefore(20);

			document.add(an);
			document.add(new Paragraph("Numarul de absente pe disciplina"));

			List<Disciplina> discipline = DisciplinaService
					.getDisciplineByAn(year);

			PdfPTable table = new PdfPTable(discipline.size() + 1); // 3
																	// columns.
			...
			switch (year) {
			case 1:
				float[] columnWidths = { 2f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f,
						1f, 1f, 1f, 1f, 1f, 1f, 1f };
				table.setWidths(columnWidths);
				break;

			case 2:
				float[] columnWidths2 = { 2f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f,
						1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f };
				table.setWidths(columnWidths2);
				break;
			case 3:
				float[] columnWidths3 = { 2f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f,
						1f, 1f, 1f };
				table.setWidths(columnWidths3);

			case 4:
				float[] columnWidths4 = { 2f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f,
						1f, 1f, 1f };
			...
			for (Disciplina disciplina : discipline) {
				...
				PdfPCell cell2 = new PdfPCell(new Phrase(disciplina
						.getNumeScurt().trim(), bf12));
			...
			for (Student student : StudentService
					.getAllStudentsByAn(AnUniversitarService.getAnByID(year))) {
				PdfPCell cell3 = new PdfPCell(new Phrase(student.getNume()
						.trim(), bf12));
				...
				for (Disciplina disc : discipline) {
					...
					int absente = 0;
					try {
						absente = RaportAnTables
								.getNumberOfAbsencesForAStudentForModule(
										student, disc);
					} finally {

						cell4 = new PdfPCell(new Paragraph(
								String.valueOf(absente)));
